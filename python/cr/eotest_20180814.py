...
seqfile = '/gpfs/slac/lsst/fs1/g/data/R_and_D/cr/sequences/crtm_itl_xed_20180630.seq'
...
if doPCTE :
    ...
    while (Lo_Volts <= Lo_End):
        vsetParLo('w',Lo_Volts)
        vsetParLo('g',Lo_Volts)
        Dphi_Volts = Dphi_Start
        while (Dphi_Volts <= Dphi_End):
            vsetDphi('w',Dphi_Volts)
            vsetDphi('g',Dphi_Volts)
...
if doFe55 :
    count = 5
    print 'Loading sequencer file ',xedfile
    loadSeq('w',xedfile)
    loadSeq('g',xedfile)
    fbase="fe55_fe55"
    ...
    for i in range(count):
        files = acquireExposure(500.0, fbase+"_%03d" % i)
    fbase="fe55_dark"
    for i in range(count):
...
if doNoise :
    count = 3
    sfile = '/gpfs/slac/lsst/fs1/g/data/R_and_D/cr/sequences/crtm_itl_ccdreset.seq'
    ...
    sfile = '/gpfs/slac/lsst/fs1/g/data/R_and_D/cr/sequences/crtm_itl_aspicreset.seq'
    print 'Loading sequencer file ',sfile
    loadSeq('w',sfile)
    loadSeq('g',sfile)
    fbase="rst"
