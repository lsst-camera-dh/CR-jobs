...
dataDir = '/gpfs/slac/lsst/fs1/g/data/R_and_D/CRTM-002/20180816/'
...
def setCCD(CCDtype):
    ...
    if CCDtype in ['itl','ITL']:
        ...
        return    
    print 'CCD = %s' % ccd
    result = raftsub.synchCommandLine(1000,"change ccdType %s" % ccd)    
...
def zeroVolts(reb):  # zeros values in memory, does not load DACs
    ...
    biases.synchCommandLine(1000,"change odP 0.01" )
...
def acquire(exptime):
    raftsub = CCS.attachSubsystem(subsystem)
    raftsub.synchCommandLine(1000,"monitor-update change taskPeriodMillis -1")  # stop monitoring REBs (noisy)
    raftsub.synchCommandLine(1000,"acquireImage")
#   raftsub.synchCommandLine(1000,"startSequencer")   # not needed with new firmware
    ...
    raftsub.synchCommandLine(1000,"monitor-update change taskPeriodMillis 1000")  # restart monitorinng REBs
...
def acquireBias(filebase):
    ...
    #if verbose: print result.getResult()
    raftsub.synchCommandLine(1000,"setExposureTime 0")
...
def acquireDark(exptime, filebase):
    ...
    #if verbose: print result.getResult()
    raftsub.synchCommandLine(1000,"setExposureTime %i" % long(exptime))
...
def acquireExposureMaster(exptime, dolight, doXED, filebase):
    if exptime == 0.0 :
        files = acquireBias(filebase)
    elif not dolight and not doXED :
        files = acquireDark(exptime, filebase)
...
def acquireExposure(exptime, filebase):
    ...
    #if verbose: print result.getResult()
    raftsub.synchCommandLine(1000,"setExposureTime %i" % long(exptime))
    raftsub.synchCommandLine(1000,"setSequencerParameter ExposureTime %i" % long(exptime * 1000 / 25))
    setFilename(filebase)
    return acquire(exptime)
...
def setSlitWidth(width):
    ...
    ts8sub = CCS.attachSubsystem(mono)
    ts8sub.synchCommandLine(1000,"setSlitSize 1 %d " % int(width))
    ts8sub.synchCommandLine(1000,"setSlitSize 2 %d " % int(width))
    time.sleep(30)
